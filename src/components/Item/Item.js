...
const Item = ({ id, name, imguno, price, brand, memory }) => {
  return (
    <div className="w-64 bg-white border shadow-2xl rounded-xl m-6">
      <img
        className="absolute bg-slate-800 h-10 w-10 p-1 rounded-full mt-2 ml-52"
        src={icon}
        alt="Imagen envio free"
      />
      <div className="h-56 rounded-t-xl bg-gray-300 flex flex-col justify-center bg-cover bg-center">
        <h1 className="absolute text-sm bg-sky-500 rounded-3xl text-white p-2 font-medium mt-52 ml-2">
          OFERTA
        </h1>
        <img
